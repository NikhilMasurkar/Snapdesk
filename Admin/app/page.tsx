...
import { canAssignRoles, canSalesMember, isSuperAdmin } from "@/lib/roles";
...
export default async function AdminPage({
  ...
  const initialTab = sp?.tab === "businesses" ? "businesses" : "dashboard";
  ...
  if (!isAdmin) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-background px-6 text-center transition-colors duration-200">
        ...
        <form action={logout}>
          <Button variant="outline" className="mt-4 h-auto rounded-xl px-6 py-2.5 text-sm font-semibold shadow-sm">
            Log out
          </Button>
        </form>
      </main>
  ...
  return (
    <AdminShell
      ...
      businessesContent={businessesContent}
    />
  );
}
